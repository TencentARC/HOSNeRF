...
class LitData(pl.LightningDataModule):
    ...
    def setup(self, stage):
        ...
        if stage == "fit" or stage is None:
            self.val_dset, self.val_dummy = self.split_each_val(
        ...
        if stage == "test" or stage is None:
            ...
            self.test_dset, self.test_dummy = self.split_each_val(
        ...
        if stage == "predict" or stage is None:
            ...
            self.predict_dset, self.pred_dummy = self.split_each_val(
    ...
    def split_each(
        ...
        _times_img = self.times[idx]
        num_rays_perimg = image_sizes_idx[0][0] * image_sizes_idx[0][1]
        _times_all = np.repeat(_times_img[:, None], num_rays_perimg, axis=1).reshape(-1)

        _masks_idx = np.concatenate([_masks[i].reshape(-1) for i in idx])
        masks_idx = _masks_idx < 1
        _rays_o = _rays_o[masks_idx]
        _rays_d = _rays_d[masks_idx]
        _viewdirs = _viewdirs[masks_idx]
        _times = _times_all[masks_idx]
        ...
        times = np.zeros((n_dset + dummy_num), dtype=np.float32)
        ...
        times[:n_dset], times[n_dset:] = _times, _times[:dummy_num]
        ...
        if _images is not None:
            ...
            images_idx = images_idx[masks_idx]
    ...
    def split_each_val(
        ...
        if _images is not None:
            ...
            _times_img = self.times[idx]
            num_rays_perimg = image_sizes_idx[0][0] * image_sizes_idx[0][1]
            _times = np.repeat(_times_img[:, None], num_rays_perimg, axis=1).reshape(-1)            
        else:
            extrinsics_idx = render_poses
            N_render = len(render_poses)
            intrinsics_idx = np.stack([self.intrinsics[0] for _ in range(N_render)])
            image_sizes_idx = np.stack([self.image_sizes[0] for _ in range(N_render)])
            _times_img = self.render_times
            num_rays_perimg = image_sizes_idx[0][0] * image_sizes_idx[0][1]
            _times = np.repeat(_times_img[:, None], num_rays_perimg, axis=1).reshape(-1)                        
    ...
    def train_dataloader(self):
        ...
        if self.batch_sampler == "single_image":
            sampler = SingleImageDDPSampler(
                ...
                N_pixels=self.bkgrays_sizes[self.i_train],
