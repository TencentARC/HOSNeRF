...
from utils.select_option import select_dataset, select_model
...
def run(
    ...
    model = select_model(model_name=model_name, cfg=cfg, basedir=basedir)
    model.logdir = logdir

    print(f"Load checkpoint from {human_path} ...")
    ckpt = pl_load(human_path)
    model.load_state_dict(ckpt['state_dict'], strict=False)       
  
    print(f"Load checkpoint from {bkgd_path} ...")
    ckpt = pl_load(bkgd_path)
    model.load_state_dict(ckpt['state_dict'], strict=False)   
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--logbase", type=str, default=None, help="folder to save results"
    )
