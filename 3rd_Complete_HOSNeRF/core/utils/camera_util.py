...
def _update_extrinsics(
    ...
    grot_vec = np.array([0., np.cos(15/180 * np.pi), np.sin(15/180 * np.pi)]) * angle
    grot_mtx = cv2.Rodrigues(grot_vec)[0].astype('float32')

    # get the transformation of smpl.
    T = np.identity(4)
    T[:-1, :-1] = grot_mtx
    if trans is not None:
        T_trans = np.identity(4)
        T_trans[:-1, -1] = -trans
        T_trans_2 = np.identity(4)
        T_trans_2[:-1, -1] = trans        
        T = T_trans_2 @ T @ T_trans
    T_smpl = np.linalg.inv(T)
    ...
    new_E_trans = extrinsics @ T_smpl

    return new_E_trans, T_smpl
...
def apply_global_tfm_to_camera(E, Rh, Th):
    ...
    return E.dot(np.linalg.inv(global_tfms)), np.linalg.inv(global_tfms)
...
def get_rays_from_KRT_bkg(H, W, K, R, T):
    r""" Sample rays on an image based on camera matrices (K, R and T)
