...
class Embedder:
    ...
    def create_embedding_fn(self):
        ...
        kick_in_iter = torch.tensor(self.kwargs['cfg'].non_rigid_motion_mlp.kick_in_iter,
                                    dtype=torch.float32)
        t = torch.clamp(self.kwargs['iter_val'] - kick_in_iter, min=0.)
        N = self.kwargs['cfg'].non_rigid_motion_mlp.full_band_iter - kick_in_iter
        m = N_freqs
        alpha = m * t / N
...
def get_embedder(multires, iter_val, cfg, is_identity=0):
    if is_identity == -1:
