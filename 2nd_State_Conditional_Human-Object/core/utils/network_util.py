...
class MotionBasisComputer(nn.Module):
    ...
    def forward(self, dst_Rs, dst_Ts, cnl_gtfms):
        ...
        inv_cnl_gtfms = torch.inverse(cnl_gtfms)
        # f_mtx_forward = torch.matmul(dst_gtfms, inv_cnl_gtfms)
        f_mtx_forward = torch.matmul(dst_gtfms, inv_cnl_gtfms)
        f_mtx_forward = f_mtx_forward.view(-1, self.total_bones, 4, 4)

        scale_Rs_forward = f_mtx_forward[:, :, :3, :3]
        Ts_forward = f_mtx_forward[:, :, :3, 3]        

        return scale_Rs, Ts, scale_Rs_forward, Ts_forward
