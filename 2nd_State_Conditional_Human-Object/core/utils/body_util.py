...
RIGHT_OBJECT_STDS = np.array([0.06, 0.06, 0.06])
LEFT_OBJECT_STDS = np.array([0.06, 0.06, 0.06])
    ...
    for joint_idx in range(0, total_joints):
        ...
        if not is_parent_joint:
            if joint_idx == SMPL_JOINT_IDX['head']:
                joint_stds = HEAD_STDS
            elif joint_idx == SMPL_JOINT_IDX['object_right']:
                joint_stds = RIGHT_OBJECT_STDS
            elif joint_idx == SMPL_JOINT_IDX['object_left']:
                joint_stds = LEFT_OBJECT_STDS
            else:
